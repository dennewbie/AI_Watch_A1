...
\begin{DoxyCode}{0}
\DoxyCodeLine{00170                                   \{}
\DoxyCodeLine{00171     rs2::pipeline pipelineStream;}
\DoxyCodeLine{00172     \textcolor{comment}{// Scaling factor for distance frame and topic name for kafka connection.}}
\DoxyCodeLine{00173     \textcolor{keywordtype}{float} scale;}
\DoxyCodeLine{00174     \textcolor{keyword}{const} \textcolor{keywordtype}{char} * destinationKafkaTopic = \textcolor{stringliteral}{"{}t1"{}};}
\DoxyCodeLine{00175     \textcolor{comment}{// Optimal resolution for D435: 848x480.}}
\DoxyCodeLine{00176     \textcolor{keywordtype}{unsigned}  \textcolor{keywordtype}{int} user\_nFrame = 1043, resX = 848, resY = 480;}
\DoxyCodeLine{00177     \textcolor{keyword}{struct }rs2\_intrinsics color\_intrin;}
\DoxyCodeLine{00178     }
\DoxyCodeLine{00179     \textcolor{comment}{// Declaring a pointer to a FacadeSingleton object (it will be helpful to easily interact with all submodules) and starting D435 camera.}}
...
\DoxyCodeLine{00181     myUtility-\/>startEnvironment(pipelineStream, color\_intrin, \& scale, resX, resY, destinationKafkaTopic);}
\DoxyCodeLine{00182 }
\DoxyCodeLine{00183     \textcolor{comment}{/*}}
\DoxyCodeLine{00184 \textcolor{comment}{        An infinite loop that captures the frames seen from the camera, gives them to OpenPose, gets the output, and builds skeletons.}}
\DoxyCodeLine{00185 \textcolor{comment}{        After that, we create a properly formatted output JSON file and send it via Kafka for further elaboration.}}
\DoxyCodeLine{00186 \textcolor{comment}{    */}}
\DoxyCodeLine{00187     \textcolor{keywordflow}{while} (\textcolor{keyword}{true}) \{}
\DoxyCodeLine{00188         myUtility-\/>getVideoFrames(user\_nFrame, pipelineStream, scale);}
\DoxyCodeLine{00189         myUtility-\/>getVideoBodyKeyPoints(\& argc, \& argv);}
\DoxyCodeLine{00190         myUtility-\/>showSkeletons(user\_nFrame);}
\DoxyCodeLine{00191         myUtility-\/>sendData(user\_nFrame);}
\DoxyCodeLine{00192         myUtility-\/>cleanBuildFolder();}
\DoxyCodeLine{00193         \textcolor{keywordflow}{break};}
\DoxyCodeLine{00194     \}}
\DoxyCodeLine{00195     \textcolor{keywordflow}{return} EXIT\_SUCCESS;}
\DoxyCodeLine{00196 \}}
