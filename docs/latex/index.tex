...
This is the AI Watch A1 documentation for C++. AI Watch A1 uses one Intel Real\+Sense D435 camera and Open\+Pose in order to achieve multi-\/person 3D skeleton detection. Once this task is completed, AI Watch A1 provides support to send each detected skeleton and its joint points\textquotesingle{} 3D coordinates (room and D435\textquotesingle{}s details have to be specified) via Kafka, in order to let further detached computation possible.\hypertarget{index_keynote_sec}{}\doxysection{Keynote 📋}\label{index_keynote_sec}
...
\begin{DoxyItemize}
\item \href{https://github.com/dennewbie/AI_Watch_A1/blob/main/caruso_denny_tesi_bsc_cs.pdf}{\texttt{ Bachelor Thesis in Computer Science}}
\end{DoxyItemize}\hypertarget{index_install_sec}{}\doxysection{Installation 🚀}\label{index_install_sec}
Note\+: Instructions for Mac\+OS with Intel CPU 1) Install \href{https://github.com/IntelRealSense/librealsense}{\texttt{ Real\+Sense SDK 2.\+0}} and its own dependencies.

2) Install \href{https://github.com/CMU-Perceptual-Computing-Lab/openpose}{\texttt{ Open\+Pose}} and its own dependencies.

3) Install \href{https://www.confluent.io/}{\texttt{ Confluent}} and its own dependencies. This step is not mandatory, but without Confluent, you will have to set up the Kafka environment.
...
9) Now let\textquotesingle{}s start the Kafka environment. Run the following commands in a new terminal session located on the parent folder of the {\ttfamily confluent} folder. Set the environment variable for the Confluent Platform home directory\+: 
\begin{DoxyCode}{0}
...
\begin{DoxyItemize}
\item if external Open\+Pose execution is chosen, then set up your Open\+Pose parameters within the \char`\"{}conf.\+conf\char`\"{} file and run the following command in your terminal\+:
\end{DoxyItemize}
