...
void UsageUtility::loadImage(cv::String imagePath, int loadType, cv::Mat & inputImage) {
    inputImage = cv::imread(imagePath, loadType);
    if (inputImage.empty()) CV_Error(LOAD_IMAGE_ERROR, LOAD_IMAGE_SCOPE);
}

void UsageUtility::saveImage (std::stringstream imageSavePath, cv::Mat & imageToSave) {
    if (!(cv::imwrite(imageSavePath.str().c_str(), imageToSave))) CV_Error(SAVE_IMAGE_ERROR, SAVE_IMAGE_SCOPE);
}

void UsageUtility::loadJSON (const char * filePathJSON, Json::Value & currentJSON) {
...
void UsageUtility::showSkeleton (unsigned int user_nFrame, Json::Value & currentJSON) {
    for (int nFrame = 0; nFrame < user_nFrame; nFrame++) {
        ...
        cv::Mat colorImage = cv::imread(colorImagePath.str(), cv::IMREAD_COLOR), distanceImage = cv::Mat(colorImage.rows, colorImage.cols, CV_32FC1);
        distanceImage = cv::imread(distanceImagePath.str(), cv::IMREAD_ANYCOLOR | cv::IMREAD_ANYDEPTH);
